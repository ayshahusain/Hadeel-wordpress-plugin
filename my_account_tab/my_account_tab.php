...
if($uri_segments[4]){
  ...
  $backurl  = home_url('/my-account/premium-support/'.$uri_segments[3]);
    ...
    $product = wc_get_product( $uri_segments[4]);
...
if(!empty($allproduct)){	
    ...
    $t_weight = $c_weight * $val; 
	$p_weight = $t_weight;
    if($c_weight_in == 'GM'){$p_weight = $t_weight / 1000;}
...
$allroow .= "<input type='hidden' id='c_weight_$ids' value='$c_weight'>
	<input type='hidden' id='c_weight_in_$ids' value='$c_weight_in'> 
	<div class='qty'><input type='hidden' id='p_weight_$ids' class='mqu' value='$p_weight' ></div>
	<tr >
	 
	<td>  <img src=' $image[0] '  style='width:50px' ></td>
	<td>".$product->get_name() ."<br>
	<snap>(Approx   $c_weight $c_weight_in   per pack)</snap></td>
<td >

	<input type='text' id='cqty$key' maxlength='4' onkeydown='return ( event.ctrlKey || event.altKey 
                    || (47<event.keyCode && event.keyCode<58 && event.shiftKey==false) 
                    || (95<event.keyCode && event.keyCode<106)
                    || (event.keyCode==8) || (event.keyCode==9) 
                    || (event.keyCode>34 && event.keyCode<40) 
                    || (event.keyCode==46) )'   onkeyup='countupdate(this.value , $ids )'    name='upqty[$name]' style='width:86px' value='$val' requied ></td>
	</tr> ";		 	 
	 }
} 
  
	  $allroow .= '</table> <span id="currentwie"></span>
	  <button type="submit" class="single_add_to_cart_button">Update</button> <a  class="button" href="'.$backurl.'">Back</a> <input type="hidden" id="validte" value="'.$product_limit.'">
	  <span id="verror"></span>
	  </form>
	  ';
      printf(
...
$orderid = $uri_segments[3]; // for www.example.com/user/account you will get 'user'
	global $wpdb;
